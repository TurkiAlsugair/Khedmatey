...
export class AuthServiceProviderService {
  ...
  async signupServiceProvider({ phoneNumber,otpCode, username, email, cities}: CreateServiceProviderDto) {
    ...
    try {
      ...
      throw new BadRequestException(`one of the cities is not supported`);
    }
    ...
    const newServiceProvider = await this.prisma.serviceProvider.create({
      data: {
        username,
        email,
        phoneNumber, //role column defaults to SERVICE_PROVIDER
        cities: {
  ...
  async updateServiceProviderInfo({ phoneNumber, username, email, cities }: UpdateServiceProviderDto) {
    ...
    if (!serviceProvider)
      throw new NotFoundException(
        `Service Provider with phone ${phoneNumber} not found`
      );

    if (serviceProvider.role !== Role.SERVICE_PROVIDER)
      throw new ForbiddenException(
        `This phone number is not a service provider`
      );
